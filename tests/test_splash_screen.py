...
class TestSplashScreenWithCoiledSnake:
    ...
    def test_draw_splash_graphics_with_perfect_snake_image(
        self, mock_exists, mock_load
    ):
    ...
    @patch("snake_game.views.renderer.pygame.image.load")
    @patch("snake_game.views.renderer.os.path.exists")
    def test_draw_splash_graphics_fallback_when_image_missing(
        self, mock_exists, mock_load
    ):
    ...
    @patch("snake_game.views.renderer.pygame.image.load")
    @patch("snake_game.views.renderer.os.path.exists")
    def test_draw_splash_graphics_fallback_when_image_load_fails(
        self, mock_exists, mock_load
    ):
    ...
    def test_draw_splash_graphics_draws_decorative_fruits(self):
        ...
        with (
            patch.object(self.renderer, "_ensure_images_loaded"),
            patch.object(
                self.renderer, "_draw_decorative_fruit_image"
            ) as mock_draw_fruit,
            ...
            assert (724, 140, FruitType.BANANA) in [
                call[0] for call in calls
            ]  # 804 - 80 = 724

            # Side areas
            assert (60, 240, FruitType.CHERRY) in [call[0] for call in calls]
            assert (744, 250, FruitType.ORANGE) in [
                call[0] for call in calls
            ]  # 804 - 60 = 744
    ...
    def test_splash_screen_with_real_image_file(self):
        ...
        with patch.object(self.renderer, "_draw_decorative_fruit_image"):
            if os.path.exists(snake_path):
                ...
                with (
                    patch("snake_game.views.renderer.pygame.image.load") as mock_load,
                    patch(
                        "snake_game.views.renderer.pygame.transform.scale"
                    ) as mock_scale,
                ):
                ...
                with patch.object(
                    self.renderer, "_draw_custom_snake_logo"
                ) as mock_custom:
